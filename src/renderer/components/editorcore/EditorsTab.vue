...
import { getFileNameFromPath } from '../../common/tools'
...
export default {
        ...
        if (thedata.path === null) {
          ...
          for (let index = 0; index < tabs.length; index++) {
            if (tabs[index].title === thedata.title) {
              _this.editableTabs[index].title =
                _this.originName[tabs[index].name] + _this.editedMark
              break
            }
          }
        } else {
    ...
    ipcRenderer.on('saveFileByButton', function(event) {
      ...
      for (let index = 0; index < tabs.length; index++) {
        if (tabs[index].name === _this.editableTabsValue) {
          ipcRenderer.send('save-file-by-button-from-tab', {
  ...
  methods: {
    ...
    handleTabsClick(thetab) {
      // 点击标签时
      let tabs = this.editableTabs
      tabs.forEach((tab, index) => {
        if (tab.name === thetab['name']) {
          ipcRenderer.send('change-file-menu-from-tab', {
            path: tab.path,
            title: tab.title
          })
        }
